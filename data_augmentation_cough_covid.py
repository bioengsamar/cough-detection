...
def Adding_white_noise(data, name):
    ...
    write('augmented_neg_covid/noise/noise_{}'.format(name), 22050, data_wn,normalized=True )
    
def Shifting_sound(data, name):
    data_roll = np.roll(data, 1600)
    write('augmented_neg_covid/shift/shifit_{}'.format(name),22050, data_roll,normalized=True)
...
def stretch_data(data, name):
    ...
    write('augmented_neg_covid/stretch/stretch_{}'.format(name), 22050, data_stretch, normalized=True)



    
def iterate_files():
    paths = Path("segmented/neg").glob('**/*.mp3')
    for path in paths:
