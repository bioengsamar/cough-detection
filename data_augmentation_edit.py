...
def Adding_white_noise(data, name):
    ...
    sf.write('augmented_cough_data/adding_noise/noise_{}'.format(name),data_wn, 22050, 'PCM_24')
    
def Shifting_sound(data, name):
    data_roll = np.roll(data, 1600)
    sf.write('augmented_cough_data/Shifting_sound/shifit_{}'.format(name),data_roll, 22050, 'PCM_24')
...
def stretch_data(data, name):
    ...
    sf.write('augmented_cough_data/stretch_sound/stretch_{}'.format(name),data_stretch, 22050, 'PCM_24')
